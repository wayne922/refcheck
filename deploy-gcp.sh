...
CLOUDSDK_METRICS_ENVIRONMENT=datacloud.antigravity gcloud run deploy $SERVICE_NAME \
  ...
  --max-instances 2 \
  --memory 512Mi \
  --set-env-vars "NODE_ENV=production,JWT_SECRET=${JWT_SECRET:-default_refcheck_secret_key_123456},AIRTABLE_API_KEY=${AIRTABLE_API_KEY},AIRTABLE_BASE_ID=${AIRTABLE_BASE_ID},SENDGRID_API_KEY=${SENDGRID_API_KEY},SENDGRID_FROM_EMAIL=${SENDGRID_FROM_EMAIL},GEMINI_API_KEY=${GEMINI_API_KEY}" \
  --quiet
