...
CLOUDSDK_METRICS_ENVIRONMENT=datacloud.antigravity gcloud run deploy $SERVICE_NAME \
  ...
  --max-instances 2 \
  --memory 512Mi \
  --set-env-vars "NODE_ENV=production,DATABASE_URL=${DATABASE_URL},JWT_SECRET=${JWT_SECRET:-default_refcheck_secret_key_123456},SENDGRID_API_KEY=${SENDGRID_API_KEY},SENDGRID_FROM_EMAIL=${SENDGRID_FROM_EMAIL},GEMINI_API_KEY=${GEMINI_API_KEY},APP_URL=${APP_URL:-https://refcheck.tech}" \
  --quiet
