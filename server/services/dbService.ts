  ...
  employers: [
    {
      ...
      companyDomain: "refcheck.tech",
      planType: "Pro",
      subscriptionStatus: "Active",
      logoUrl: "",
      brandColour: "#2E7EBF",
      brandedSenderName: "RefCheck Recruitment",
      apiKey: "dev-refcheck-api-key",
...
export const dbService = {
  ...
  deleteCandidate: async (id: string) => {
    ...
    try {
      ...
      const referees = await dbService.getRefereesForCandidate(id);
  ...
  duplicateQuestionnaireTemplate: async (id: string, createdByUserId: string) => {
    const source = await dbService.getQuestionnaireTemplate(id);
    if (!source) throw new Error("Source template not found");

    return dbService.createQuestionnaireTemplate({
  ...
  getResponsesForReferee: async (refereeId: string) => {
    ...
    try {
      ...
      console.error(`Database error fetching responses for referee ${refereeId}:`, err);
      throw err;
    }
  }
};

export const airtableService = dbService;
