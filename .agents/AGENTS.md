...
- **Database / API Core**: Google Cloud SQL native PostgreSQL (`34.151.99.147`) managed via `server/db.ts` and `server/services/dbService.ts`.
- **Integrations**: Gemini / OpenAI API for AI evaluations, SendGrid for emails, Twilio for SMS, PDFKit (`pdfkit`) and `pdf-parse` for PDF reports.
- **Styling**: Tailwind CSS (v3.x), Lucide React icons, Clsx.

## Project Structure & Architecture
- **Client**: Located in [client/](file:///Users/waynesullivan/antigravity/Compliance/refcheck/client). Ensure all React frontend views and components go here.
- **Server**: Located in [server/](file:///Users/waynesullivan/antigravity/Compliance/refcheck/server). Express configurations and endpoints are set in [server/index.ts](file:///Users/waynesullivan/antigravity/Compliance/refcheck/server/index.ts).
- **Services**: Located in [server/services/](file:///Users/waynesullivan/antigravity/Compliance/refcheck/server/services). Contains modules for database operations, Gemini/OpenAI models, emails, and SMS.
  - [dbService.ts](file:///Users/waynesullivan/antigravity/Compliance/refcheck/server/services/dbService.ts) encapsulates Google Cloud SQL PostgreSQL operations and candidate/referee queries.
...
2. **Data Model Updates**: Schema definition changes are applied through Cloud SQL PostgreSQL tables. Update service queries in [dbService.ts](file:///Users/waynesullivan/antigravity/Compliance/refcheck/server/services/dbService.ts) to match schema updates.
